    ...
    def forward(self, normed_density, unnormed_density, points):
        ...
        for idx, im_points in enumerate(points):
            if len(im_points) > 0:
                ...
                if self.norm_cood:
                    im_points = im_points / self.c_size * \
                        2 - 1  # map to [-1, 1]
                x = im_points[:, 0].unsqueeze_(1)  # [#gt, 1]
                y = im_points[:, 1].unsqueeze_(1)
                # [#gt, #cood]
                x_dis = -2 * torch.matmul(x, self.cood) + \
                    x * x + self.cood * self.cood
                y_dis = -2 * torch.matmul(y, self.cood) + \
                    y * y + self.cood * self.cood
                y_dis.unsqueeze_(2)
                x_dis.unsqueeze_(1)
                dis = y_dis + x_dis
                ...
                target_prob = (torch.ones(
                    [len(im_points)]) / len(im_points)).to(self.device)
                # use sinkhorn to solve OT, compute optimal beta.
                P, log = sinkhorn(target_prob, source_prob, dis,
